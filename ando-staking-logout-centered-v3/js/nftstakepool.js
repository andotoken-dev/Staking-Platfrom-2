...
async function initializing() {
    ...
    masterChefNFT = await new web3.eth.Contract(ABI_NFT_Rare_MASTERCHEF, ADDRESS_NFT_Rare_MASTERCHEF);
    ...
    masterChefNFT = await new web3.eth.Contract(ABI_NFTSTAKE, ADDRESS_NFT_Stake); //NFT Stake Pool
...
function updateRewards(){
    //Check Rare First
    try {
        masterChefNFT.methods.pendingToken(0,currentAddr).call().then(
            res => {
                isStaker = res
    ...
    //Check Legendary
        try {
            masterChefNFT.methods.pendingToken(1,currentAddr).call().then(
                res => {
                    isStaker = res;
                    var validStaker;
                    if (isStaker === 0 || isStaker === "0"){
        ...
        //end of legendary check
            //Check Mythic
            try {
                masterChefNFT.methods.pendingToken(2,currentAddr).call().then(
                    res => {
                        isStaker = res;
                        var validStaker;
                        if (isStaker === 0 || isStaker === "0"){
...
async function updateRare() {
    if (AirNFT && currentAddr !== null && currentAddr !== undefined && currentAddr !== "") {
        ...
        try {
            AirNFT.methods.isApprovedForAll(currentAddr, ADDRESS_NFT_Stake).call().then(
                res => {
                    isApproved = res;
                    var ApproveThis;
        ...
        masterChefNFT.methods.poolInfo(0).call().then(
            res => {
                $("#rare-api").text((res.annualReward / 1e18).toFixed(8));
                $("#nft-rare-locking-time").text((res.poolTime / 86400 + " days"));

            }
        )

        masterChefNFT.methods.treasure().call().then(
            response => {
        ...
        if (masterChefNFT  && currentAddr !== null && currentAddr !== undefined && currentAddr !== "") {
            try{
                masterChefNFT.methods.pendingToken(0,currentAddr).call().then(
                    res => {
                ...
                masterChefNFT.methods.depositOf(0,currentAddr).call().then(
                    res=>{
                        rarestaked = res
...
rareButtonEnable.addEventListener('click', () => {
    if (AirNFT && currentAddr !== null && currentAddr !== undefined && currentAddr !== "") {
        try {
            AirNFT.methods.setApprovalForAll(ADDRESS_NFT_Stake, true).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err)
...
rareCollect.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            masterChefNFT.methods.harvest(0).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
rareUnstakingConfirm.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            var NFTID = [tokenid]
            masterChefNFT.methods.withdraw(0).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
rareStakingConfirm.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            var NFTID = [tokenid]
            masterChefNFT.methods.deposit(0,NFTID).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
async function updateMythic() {
    if (AirNFT && currentAddr !== null && currentAddr !== undefined && currentAddr !== "") {
        ...
        masterChefNFT.methods.poolInfo(2).call().then(
            res => {
                $("#mythic-api").text((res.annualReward / 1e18).toFixed(8));
                $("#nft-mythic-locking-time").text((res.poolTime / 86400 + " days"));
            }
        )
        ...
        if (masterChefNFT  && currentAddr !== null && currentAddr !== undefined && currentAddr !== "") {
            try{
                ...
                masterChefNFT.methods.depositOf(2,currentAddr).call().then(
                    res=>{
                        mythicstaked = res
...
mythicCollect.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            masterChefNFT.methods.harvest(2).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
rewardsCheckmythic.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            var NFTID = tokenid
                masterChefMythic.methods.pendingToken(2,currentAddr).call().then(
                    res=>{
                        rewardsTotal = res / 1e18;
...
mythicStakingConfirm.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            var NFTID = [tokenid]
            masterChefNFT.methods.deposit(2,NFTID).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
mythicUnstakingConfirm.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            var NFTID = [tokenid]
            masterChefNFT.methods.withdraw(2).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
async function updateLegendary() {
    if (AirNFT && currentAddr !== null && currentAddr !== undefined && currentAddr !== "") {
        ...
        masterChefNFT.methods.poolInfo(1).call().then(
            res => {
                $("#legendary-api").text((res.annualReward / 1e18).toFixed(8));
                $("#nft-legendary-locking-time").text((res.poolTime / 86400 + " days"));
            }
        )
        ...
        if (masterChefNFT  && currentAddr !== null && currentAddr !== undefined && currentAddr !== "") {
            try{
                ...
                masterChefNFT.methods.depositOf(1,currentAddr).call().then(
                    res=>{
                        legendarystaked = res
...
legendaryCollect.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            masterChefNFT.methods.harvest(1).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
legendaryStakingConfirm.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            var NFTID = [tokenid]
            masterChefNFT.methods.deposit(1,NFTID).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
...
legendaryUnstakingConfirm.addEventListener('click', () => {
    try {
        if (masterChefNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            var NFTID = [tokenid]
            masterChefNFT.methods.withdraw(1).send({
                value: 0,
                from: currentAddr,
            }).once("error", (err) => {
                console.log(err);
