...
async function mainContractInfo() {
    ...
    masterChef = await new web3.eth.Contract(ABI_NFT_Mythic_MASTERCHEF, ADDRESS_NFT_Mythic_MASTERCHEF); //NFT Staking platform
    tokenNFTL = await new web3.eth.Contract(ABI_AirNFT, ADDRESS_AirNFT); //NFT Limited
    masterChefNFTL = await new web3.eth.Contract(ABI_NFT_Mythic_MASTERCHEF, ADDRESS_NFT_Mythic_MASTERCHEF); //NFT Limited Staking platform
...
async function updateParameters() {
    ...
    if (tokenNFT) {
        ...
        try {
            tokenNFT.methods.isApprovedForAll(currentAddr, ADDRESS_NFT_Mythic_MASTERCHEF).call().then(res => {
    ...
    if (currentAddr != null && currentAddr != undefined && currentAddr != "") {
            ...
            masterChef.methods.userStakeInfo(currentAddr).call().then(res => {
                ...
                $("#mythic-total").text(totalTokenInFarm15 + " USD");
                $("#mythic-api").text(tokenPerBlock + "");
            })
        }catch (error) {
            console.log (error)          
}
       ...
       if (currentAddr != null && currentAddr != undefined && currentAddr != "") {
         ...
         console.log (error)
          ...
          masterChef.methods.balanceOf(currentAddr).call().then(res => {
              ...
              $("#mythic-staked1").text((res));

              var lastDeposit = res.timeOfLastUpdate;
              masterChef.methods.stakers(currentAddr).call().then(r => {
                 $("#mythic-locking-time").text((r.timeOfLastUpdate  / 60 / 60 / 24).toFixed(0) + " days");
                  if(lastDeposit == 0){
                      $("#mythic-unstake-warning").css("display", "none");
                  }else{
                      ...
                      if(Date.now()/1000 >= unlockTimeStamp){
                          ...
                          $("#mythic-unstake-date").text(date + "/" + month + "/" + year + " - " + hour + ":" + min);
                          $("#mythic-unstake-warning").css("display", "block");
                      }
                  }
              })
    ...
    try {
        if (tokenNFT && currentAddr != null && currentAddr != undefined && currentAddr != "") {
           // tokenNFT.methods.setApprovalForAll(currentAddr, true).send({
            ...
            //})
            tokenNFT.methods.setApprovalForAll(ADDRESS_NFT_Mythic_MASTERCHEF, true).send({
                value: 0,
                from: currentAddr,
...
$("#mythic-unstake-max").click(() => {
    $('#mythic-input-unstake').val(yourTokenInFarm15);
});
...
$("#mythic-stake-max").click(() => {
    $('#mythic-input-stake').val(yourLpBalance.toFixed(0));
});
